...
function ContextMenu(contextContainerID, menuItemClickCallback, options)
{
    ...
    this.contextMenuContainer.click(function(e){
        ...
        if (!isElementInViewport(contextMenu)) {
            ...
            var showPos = menuPos.left - leftShiftOver - 100; //Subtract an extra 10 so it has some margin space

            contextMenu.css({top: menuPos.top - 100, left: showPos - 100, position: 'absolute'});
        }
        else
        {
            contextMenu.css({top: menuPos.top - 100, left: menuPos.left - 100, position: 'absolute'});
        }
